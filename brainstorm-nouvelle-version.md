...
- **Montant d'une enveloppe = somme de ses contributions.** Une seule règle, y compris pour un livret dédié. Le solde du compte sert de contrôle via le recalage mensuel (création de mois) : contribution d'ajustement tracée (intérêts / arrondis). Pas de mode « enveloppe = solde du compte ».
...
- **Suppression guidée** : une enveloppe avec un historique n'est jamais supprimée — elle se **clôture**, seule (l'argent redevient hors enveloppes du hôte) ou en **réaffectant** son contenu vers n'importe quelle enveloppe ouverte ou un compte actif (virement système si l'argent change de compte). La suppression définitive n'existe que pour une enveloppe sans historique. Interdite dans tous les cas si elle alimente une ligne mensualisée (« démensualisez d'abord »).
- **Désactivation de compte** (`is_active`) : la voie propre quand un compte ne sert plus — il disparaît des saisies, du bilan, du patrimoine et du prefill des mois, mais tout l'historique reste et il est réactivable (bas de la page Comptes). Solde connu ≠ 0 → virer d'abord vers un compte actif (virement système) ; enveloppe ouverte hébergée → la clôturer/déplacer d'abord ; compte principal → jamais désactivable. La **suppression définitive** n'existe que pour un compte sans aucun historique (créé par erreur).
- **Déplacement d'enveloppe** vers un autre compte hôte : l'argent suit physiquement → virement système ancien hôte → nouvel hôte (confirmé par modal), refusé sans mois ouvert.
- **Découvert autorisé** (case par compte) : exempte le compte de l'invariant Σ enveloppes ≤ solde ; le hors-enveloppes peut devenir négatif, affiché en ambre au lieu de bloquer.
- **Invariant** (01/09/2026) : Σ enveloppes ouvertes d'un compte ≤ solde du compte (quand il est connu). Montant initial et contributions virtuelles refusés au-delà du « hors enveloppes » (rappel du disponible dans le formulaire) ; option « pris dans une autre enveloppe du compte » et **réaffectation** entre enveloppes d'un même compte (deux contributions liées, aucun mouvement bancaire). Ajustements (recalage) et réaffectations sont les voies sanctionnées.
- **Recalage** : uniquement dans « Nouveau mois » (montant réel de chaque enveloppe, « = compte » pour recopier le solde saisi) — le bouton « Recaler sur le compte » de la page Comptes a été retiré (01/09 : trompeur sur un compte courant, où l'écart est simplement le hors-enveloppes). Import Mongo : script `align-envelopes.mjs` (initiale = solde du premier mois, ajustements mensuels).
- **Dépense depuis une enveloppe** (cas réel : payer l'hôtel du Japon avant l'échéance) : se saisit **dans le mois** comme une entrée, avec « depuis l'enveloppe X » et la case « fait partie de l'objectif ». Contribution *dépense* liée et synchronisée (l'enveloppe baisse, le compte baisse via l'entrée, le hors-enveloppes ne bouge pas). Cible affichée corrigée des dépenses du projet : 2 720 / 3 500 (4 500 − 1 000 dépensés), reste à épargner et mensualité inchangés ; case décochée → cible intacte (c'est un trou).
...
- **Plus de fixe / variable** (révisé le 01/09/2026 après test en conditions réelles : la distinction induisait en erreur — une ligne d'épargne ressemblait à une facture sans que l'utilisateur sache pourquoi). Règle unique :
  ...
  - **☐ payé** sur toute ligne qui a un prévu et aucune entrée (héritée du template ou ajoutée au mois avec un prévu) : cocher crée l'entrée au prévu, datée du *jour du mois* de la ligne sinon d'aujourd'hui. La case est **à sens unique** : dès qu'une entrée existe elle disparaît ; pour annuler on supprime les entrées (×) — supprimer la dernière entrée d'une ligne supprime aussi ses virements système liés et fait revenir la case si le prévu reste ;
